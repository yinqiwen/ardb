	...
	int ArdbServer::ParseConfig(const Properties& props, ArdbServerConfig& cfg)
	{
		...
		int ArdbServer::Config(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			else if (arg0 == "get")
			{
				...
				{
					if (fnmatch(cmd.GetArguments()[1].c_str(),
					        it->first.c_str(), 0) == 0)
					{
		...
		int ArdbServer::SetEX(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			m_db->SetEx(ctx.currentDB, cmd.GetArguments()[0],
			        cmd.GetArguments()[2], secs);
			fill_status_reply(ctx.reply, "OK");
			return 0;
		}
		...
		int ArdbServer::SetRange(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			int ret = m_db->SetRange(ctx.currentDB, cmd.GetArguments()[0],
			        offset, cmd.GetArguments()[2]);
			fill_int_reply(ctx.reply, ret);
			return 0;
		}
		...
		int ArdbServer::SetBit(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			uint8 bit = cmd.GetArguments()[2] != "1";
		...
		int ArdbServer::IncrbyFloat(ArdbConnContext& ctx,
		...
		int ArdbServer::Incrby(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			int ret = m_db->Incrby(ctx.currentDB, cmd.GetArguments()[0],
			        increment, val);
			if (ret == 0)
			{
		...
		int ArdbServer::GetBit(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			int ret = m_db->GetBit(ctx.currentDB, cmd.GetArguments()[0],
			        offset);
			fill_int_reply(ctx.reply, ret);
			return 0;
		}
		...
		int ArdbServer::Decrby(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			int ret = m_db->Decrby(ctx.currentDB, cmd.GetArguments()[0],
			        decrement, val);
			if (ret == 0)
			{
				fill_int_reply(ctx.reply, val);
		...
		int ArdbServer::Bitcount(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			if (cmd.GetArguments().size() == 1)
			{
				count = m_db->BitCount(ctx.currentDB, cmd.GetArguments()[0], 0,
				        -1);
			}
			else
			{
				...
				count = m_db->BitCount(ctx.currentDB, cmd.GetArguments()[0],
				        start, end);
			}
			fill_int_reply(ctx.reply, count);
			return 0;
		...
		{
			m_db->HSet(ctx.currentDB, cmd.GetArguments()[0],
		...
		int ArdbServer::HIncrbyFloat(ArdbConnContext& ctx,
		        RedisCommandFrame& cmd)
		{
		...
		int ArdbServer::HGetAll(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			StringArray fields;
			ValueArray results;
			m_db->HGetAll(ctx.currentDB, cmd.GetArguments()[0], fields,
			        results);
			ctx.reply.type = REDIS_REPLY_ARRAY;
...
//========================Set CMDs==============================
		int ArdbServer::SAdd(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			int count = m_db->SAdd(ctx.currentDB, cmd.GetArguments()[0],
			        values);
			fill_int_reply(ctx.reply, count);
			return 0;
		}
		...
		int ArdbServer::SDiffStore(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			ValueSet vs;
			int ret = m_db->SDiffStore(ctx.currentDB, cmd.GetArguments()[0],
			        keys);
			fill_int_reply(ctx.reply, ret);
			return 0;
		}
		...
		int ArdbServer::SInterStore(ArdbConnContext& ctx,
		        RedisCommandFrame& cmd)
		{
			SliceArray keys;
			for (uint32 i = 1; i < cmd.GetArguments().size(); i++)
			...
			ValueSet vs;
			int ret = m_db->SInterStore(ctx.currentDB, cmd.GetArguments()[0],
			        keys);
			fill_int_reply(ctx.reply, ret);
			return 0;
		}
		...
		int ArdbServer::SRandMember(ArdbConnContext& ctx,
		        RedisCommandFrame& cmd)
		{
			ValueArray vs;
			int32 count = 1;
		...
		int ArdbServer::SUnionStore(ArdbConnContext& ctx,
		        RedisCommandFrame& cmd)
		{
			SliceArray keys;
			for (uint32 i = 1; i < cmd.GetArguments().size(); i++)
			...
			int ret = m_db->SUnionStore(ctx.currentDB, cmd.GetArguments()[0],
			        keys);
			fill_int_reply(ctx.reply, ret);
			return 0;
		}

		int ArdbServer::SUnionCount(ArdbConnContext& ctx,
		        RedisCommandFrame& cmd)
		{
			SliceArray keys;
			for (uint32 i = 0; i < cmd.GetArguments().size(); i++)
		...
		int ArdbServer::SInterCount(ArdbConnContext& ctx,
		        RedisCommandFrame& cmd)
		{
			SliceArray keys;
			for (uint32 i = 0; i < cmd.GetArguments().size(); i++)
...
//===========================Sorted Sets cmds==============================
		int ArdbServer::ZAdd(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			{
				ValueArray vs;
				m_db->ZAddLimit(ctx.currentDB, cmd.GetArguments()[0], scores,
				        svs, limit, vs);
				fill_array_reply(ctx.reply, vs);
			}
			else
			{
				int count = m_db->ZAdd(ctx.currentDB, cmd.GetArguments()[0],
				        scores, svs);
				fill_int_reply(ctx.reply, count);
			}
			return 0;
		...
		int ArdbServer::ZRange(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			{
				if (strcasecmp(cmd.GetArguments()[3].c_str(), "withscores")
				        != 0)
				{
					fill_error_reply(ctx.reply, "ERR syntax error");
					return 0;
		...
		int ArdbServer::ZRangeByScore(ArdbConnContext& ctx,
		        RedisCommandFrame& cmd)
		{
			QueryOptions options;
			if (cmd.GetArguments().size() >= 4)
			{
				bool ret = process_query_options(cmd.GetArguments(), 3,
				        options);
				if (!ret)
				{
					fill_error_reply(ctx.reply, "ERR syntax error");
		        ...
		        RedisCommandFrame& cmd)
		{
			...
			int count = m_db->ZRemRangeByRank(ctx.currentDB,
			        cmd.GetArguments()[0], start, stop);
			fill_int_reply(ctx.reply, count);
			return 0;
		}
		...
		int ArdbServer::ZRemRangeByScore(ArdbConnContext& ctx,
		        RedisCommandFrame& cmd)
		{
			int count = m_db->ZRemRangeByScore(ctx.currentDB,
			        cmd.GetArguments()[0], cmd.GetArguments()[1],
			        cmd.GetArguments()[2]);
			fill_int_reply(ctx.reply, count);
			return 0;
		}
		...
		int ArdbServer::ZRevRange(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			options.withscores = withscores;
			ValueArray vs;
			m_db->ZRevRange(ctx.currentDB, cmd.GetArguments()[0], start, stop,
			        vs, options);
			fill_array_reply(ctx.reply, vs);
			return 0;
		}
		...
		int ArdbServer::ZInterStore(ArdbConnContext& ctx,
		        RedisCommandFrame& cmd)
		{
			SliceArray keys;
			WeightArray ws;
		...
		int ArdbServer::ZUnionStore(ArdbConnContext& ctx,
		        RedisCommandFrame& cmd)
		{
			SliceArray keys;
			WeightArray ws;
...
//=========================Lists cmds================================
		int ArdbServer::LIndex(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			std::string v;
			int ret = m_db->LIndex(ctx.currentDB, cmd.GetArguments()[0], index,
			        v);
			if (ret < 0)
			{
				ctx.reply.type = REDIS_REPLY_NIL;
...
//=========================Tables cmds================================
		...
		int ArdbServer::TGet(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			ValueArray vs;
			std::string err;
			int ret = m_db->TGet(ctx.currentDB, cmd.GetArguments()[0], options,
			        vs, err);
			if (ret == 0)
			{
				fill_array_reply(ctx.reply, vs);
		...
		int ArdbServer::TUpdate(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			int ret = m_db->TUpdate(ctx.currentDB, cmd.GetArguments()[0],
			        options);
			if (ret >= 0)
			{
				fill_int_reply(ctx.reply, ret);
			...
			{
				fill_error_reply(ctx.reply,
				        "ERR no record found for condition");
			}
			return 0;
		}
		...
		int ArdbServer::TInsert(ArdbConnContext& ctx, RedisCommandFrame& cmd)
		{
			...
			int ret = m_db->TInsert(ctx.currentDB, cmd.GetArguments()[0],
			        options, cmd.GetCommand() == "treplace", err);
			if (ret != 0)
			{
				ctx.reply.str = err;
...
//	}
		        ...
		        RedisCommandFrame& args)
		{
			...
			RedisCommandHandlerSetting* setting =
			        FindRedisCommandHandlerSetting(cmd);
		...
		static void conn_pipeline_finallize(ChannelPipeline* pipeline,
		        void* data)
		{
		...
		int ArdbServer::Start(const Properties& props)
		{
			...
			if (!m_cfg.listen_host.empty())
			{
				...
				ServerSocketChannel* server =
				        m_service->NewServerSocketChannel();
