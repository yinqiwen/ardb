...
ArdbServer::ArdbServer(KeyValueEngineFactory& engine) :
		m_service(NULL), m_db(NULL), m_engine(engine), m_slowlog_handler(m_cfg), m_repli_serv(
...
int ArdbServer::PSetEX(ArdbConnContext& ctx, RedisCommandFrame& cmd)
{
	...
	m_db->PSetEx(ctx.currentDB, cmd.GetArguments()[0], cmd.GetArguments()[2],
			mills);
	fill_status_reply(ctx.reply, "OK");
	return 0;
}
...
int ArdbServer::IncrbyFloat(ArdbConnContext& ctx, RedisCommandFrame& cmd)
{
	...
	int ret = m_db->IncrbyFloat(ctx.currentDB, cmd.GetArguments()[0], increment,
			val);
	if (ret == 0)
	{
...
int ArdbServer::HIncrby(ArdbConnContext& ctx, RedisCommandFrame& cmd)
{
	...
	m_db->HIncrby(ctx.currentDB, cmd.GetArguments()[0], cmd.GetArguments()[1],
			increment, val);
	fill_int_reply(ctx.reply, val);
	return 0;
}
...
int ArdbServer::ZInterStore(ArdbConnContext& ctx, RedisCommandFrame& cmd)
{
	...
	int count = m_db->ZInterStore(ctx.currentDB, cmd.GetArguments()[0], keys,
			ws, type);
	fill_int_reply(ctx.reply, count);
	return 0;
}
...
int ArdbServer::ZUnionStore(ArdbConnContext& ctx, RedisCommandFrame& cmd)
{
	...
	int count = m_db->ZUnionStore(ctx.currentDB, cmd.GetArguments()[0], keys,
			ws, type);
	fill_int_reply(ctx.reply, count);
	return 0;
}
...
{
	std::string v;
	m_db->RPopLPush(ctx.currentDB, cmd.GetArguments()[0], cmd.GetArguments()[1],
			v);
	fill_str_reply(ctx.reply, v);
	return 0;
}
