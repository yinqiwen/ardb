...
namespace rddb
{
	...
	int RDDB::GetBit(DBID db, const Slice& key, int bitoffset)
	{
		KeyObject k(key);
		ValueObject v;
		if (GetValue(db, k, &v) < 0)
	...
	int RDDB::BitOP(DBID db, const Slice& opstr, const Slice& dstkey,
			SliceArray& keys)
	{
		long op, j, numkeys;
		unsigned char **src;
	...
	int RDDB::BitCount(DBID db, const Slice& key, int start, int end)
	{
		KeyObject k(key);
		ValueObject v;
		if (GetValue(db, k, &v) < 0)
