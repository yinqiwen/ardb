...
namespace ardb
{
    ...
    Master::Master(ArdbServer* server) :
            m_server(server), m_notify_channel(NULL), m_backlog(
                    server->m_repl_backlog), m_dumping_db(false), m_dumpdb_offset(
    ...
    {
        /*
         * Clear dbid first. After full resynced, the first syncing
    ...
    {
        std::string dump_file_path = m_server->m_cfg.repl_data_dir
                + "/dump.rdb";
            ...
            MessageEvent<RedisCommandFrame>& e)
    {
        ...
        if (!strcasecmp(cmd->GetCommand().c_str(), "replconf"))
        {
            if (cmd->GetArguments().size() == 2
                    && !strcasecmp(cmd->GetArguments()[0].c_str(), "ack"))
            {
                int64 offset;
                if (string_toint64(cmd->GetArguments()[1], offset))
                {
                    SlaveConnTable::iterator found = m_slave_table.find(
    ...
    void Master::AddSlave(Channel* slave, RedisCommandFrame& cmd)
    {
        ...
        } else
        {
            ...
            } else
            {
                ...
                {
                    std::vector<std::string> ss = split_string(
                            cmd.GetArguments()[2], "|");
    ...
    void Master::PrintSlaves(std::string& str)
    {
        ...
        while (it != m_slave_table.end())
        {
            ...
            Address* remote =
                    const_cast<Address*>(slave->conn->GetRemoteAddress());
            std::string address;
            ...
            sprintf(buffer, "slave%u:%s,state=%s,"
                    "offset=%"PRId64",lag=%u\r\n", i, address.c_str(), state,
                    slave->sync_offset, lag);
            it++;
            i++;
            str.append(buffer);
        }
