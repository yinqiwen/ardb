...
namespace ardb
{
	...
	void ZookeeperClient::RecursiveCreateCB(int rc, const char *value, const void *data)
	{
		...
		if (rc == ZOK || rc == ZNODEEXISTS)
		{
			...
			} else
			{
				...
				zoo_acreate(cb->zk->m_zk, first_dir.c_str(), need_value ? cb->value.data() : NULL, need_value ? cb->value.size() : 0, &ZOO_OPEN_ACL_UNSAFE, flags, ZookeeperClient::RecursiveCreateCB, cb);
			}
		} else
		{
	...
	int ZookeeperClient::Create(const std::string& path, const std::string& value, const ZKACLArray& acls, int flags, bool recursive)
	{
		if (NULL == m_zk)
		{
			return -1;
		...
		if (recursive)
		{
			...
			return zoo_acreate(m_zk, first_dir.c_str(), need_value ? value.data() : NULL, need_value ? value.size() : 0, &ZOO_OPEN_ACL_UNSAFE, need_value ? flags : 0, ZookeeperClient::RecursiveCreateCB, cbdata);
		}
		std::pair<std::string, ZookeeperClient*>* cb = new std::pair<std::string, ZookeeperClient*>;
		cb->first = path;
		cb->second = this;
