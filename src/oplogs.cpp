...
void OpLogs::ReOpenOpLog()
{
	if (NULL == m_op_log_file)
	{
		...
		if (NULL == m_op_log_file)
		{
			ERROR_LOG( "Failed to open oplog file:%s", oplog_file_path.c_str());
		}
	}
}
...
void OpLogs::WriteCachedOp(uint64 seq, CachedOp* op)
{
	...
	else
	{
		...
		{
			BufferHelper::WriteVarString(tmp, *(cmdOp->cmd->GetArgument(i)));
		}
	}
...
CachedOp* OpLogs::SaveCmdOp(RedisCommandFrame* cmd, bool writeOpLog)
{
	...
	while (m_mem_op_logs.size() >= m_server->GetServerConfig().rep_backlog_size)
	{
		RemoveOldestOp();
	}
	if (writeOpLog)
...
int OpLogs::LoadOpLog(DBIDSet& dbs, uint64& seq, Buffer& buf, bool is_master_slave)
{
